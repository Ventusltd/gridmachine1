# The runners, sealed as a capsule

Sealed 2026-09-07T23:01:12.612Z. What the runners are, what they cost, how they are built, and what they taught. Everything here was measured on this machine tonight, not assumed.

## The parts

| file | sha256 (first 16) | bytes |
| --- | --- | --- |
| `overnight.mjs` | `e1e35dca659f5102` | 10,853 |
| `smoke.mjs` | `f16fcbf394443f44` | 4,241 |
| `pair-runner.mjs` | `641080848f4171bc` | 21,962 |

They live on the SSD at `D:/gridatlas-ci/`. This capsule is the record; the files are the thing.

## What a browser session actually costs

The single most useful number of the night, and it overturned what both of us believed:

| condition | CPU average across 20 cores | peak | free RAM low |
| --- | --- | --- | --- |
| Machine idle | 3 to 6% | | |
| **One** browser session on the map | **51.6%** | 59.9% | 2.68 GB |
| Two sessions | 84.8% | 97.1% | 2.03 GB |
| One session with raster, occlusion and paint limits | 50.6% | 58.8% | 2.38 GB |

**One session costs half the machine.** Thread and paint flags moved it by one point, which is noise, so the cost is the map's own continuous render loop rather than anything Chromium can be told to skip.

**Therefore the lever is duration, not concurrency.** A study that opens a session, measures for fifteen seconds and closes it costs almost nothing. Five hours of sessions held open back to back is what made the fans scream, and that is a different activity from the same tool.

## How the runners are shaped

- **One runner, one study, one session, closed before the next begins.** No batches, no lingering browser, memory returns to the machine each time.
- **Memory governor**, measured rather than assumed: a floor below which a study will not start, a reserve left for the host, and a per-copy cost re-measured after every batch. The measured cost of a copy was 0.41 GB, against the 1.2 GB I had guessed.
- **Thermal guard by proxy.** This firmware exposes no temperature sensor, so the runner reads the processor clock against its rated maximum and waits two minutes below three quarters of rated.
- **Five shapes in every study, always together:** Pixel 7 portrait and landscape, Galaxy S9+ portrait and landscape, and desktop. Android and desktop in one run, never one or the other. The device profiles ship with the driver and carry the real user agent, pixel ratio and touch flags rather than viewport numbers anyone invented.
- **A real handset needs no emulator.** `adb` is installed at `D:/android/platform-tools` (7.7 MB download, 19 MB on disk) and the driver can drive Chrome on a plugged-in phone. The phone renders, so the laptop stays cool. An Android Studio emulator would have cost gigabytes and burned the very budget this capsule exists to protect.

## What the runners taught, in one night

1. **A test can produce a vacuous pass as easily as product code can.** The first layers study read the label after the tap with a stricter matcher than before it, found nothing, and called a string differing from null a change. It reported six shapes working when the control was broken on four. **Measure both sides identically or the comparison is worthless.**
2. **A repeat is not evidence after the first few.** 615 identical passes across five hours confirmed one fact many times. The findings that changed the software came from analysis and from one screenshot, not from repetition.
3. **A floor is not a target.** The runner stopped as soon as it touched its 20% CPU floor, then slept thirteen minutes in every fifteen. Measured duty was about 10%.
4. **A constant chosen once cannot know what else the machine is doing.** Four copies on a 2.5 GB reserve took the host under and it killed its own background tasks. A governor that measures both sides of every batch cannot drift that way for more than one batch.
5. **Small viewports are not big ones scaled down.** The layers control fires on a Pixel in landscape and does not fire on a Galaxy S9+ in landscape, at 658 by 320. Testing one phone shape would have missed it.
...
## How to rebuild from this capsule

Take the three files at the hashes above. They need one dependency, already present: the browser driver at version 1.58.2 with its Chromium. Nothing else is installed, no packages were added, and no service is required. A study is a function that opens a session, measures, closes it and writes a receipt; adding a ticket means adding a study, not changing the runner.
